...
export async function POST(request: Request, { params }: { params: Promise<{ orderNumber: string }> }) {
  try {
    const [user, input, { orderNumber }] = await Promise.all([
      getCustomerApiUser(request), request.json() as Promise<{ trackingToken?: string }>, params,
    ]);
    return Response.json(await resumeMobilePayment(orderNumber, user?.id, input.trackingToken));
