...
export const getRuntimeStoreConfig = cache(async () => {
  ...
  return {
    ...
    contact: { ...storeConfig.contact, email: settings?.email?.trim() || null, phone: settings?.phone?.trim() || null, address: settings?.street && settings.postalCode && settings.city ? { street: settings.street, postalCode: settings.postalCode, city: settings.city } : null, social: { ...storeConfig.contact.social, instagram: settings?.instagramUrl || null, facebook: settings?.facebookUrl || null } },
