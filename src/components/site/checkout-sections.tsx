...
export type CheckoutUser = {
  ...
  email?: string | null;
  phone?: string | null;
  address?: { street: string; streetExtra?: string | null; postalCode: string; city: string } | null;
};
export type Quote = { subtotalRappen: number; discountRappen: number; deliveryFeeRappen: number; totalRappen: number };
export type FieldErrors = Partial<Record<string, string>>;
export type FulfillmentType = "DELIVERY" | "PICKUP";
export type PaymentMethod = "STRIPE" | "CASH_ON_DELIVERY" | "PAY_AT_PICKUP";

const radioCard = "flex min-h-14 cursor-pointer items-center gap-3 rounded-xl border border-border bg-surface px-4 py-3 has-[:checked]:border-primary has-[:checked]:ring-2 has-[:checked]:ring-primary/15";
...
export function AddressFields({ de, user, errors, refreshQuote }: { de: boolean; user?: CheckoutUser; errors: FieldErrors; refreshQuote: () => void }) {
  return (
    <Card className="grid gap-4 rounded-none p-5 sm:grid-cols-2"><h2 className="font-display text-xl font-bold sm:col-span-2">{de ? "Lieferadresse" : "Delivery address"}</h2>
      ...
      <Field id="city" label={de ? "Ort" : "City"} error={errors.city}><Input id="city" name="city" autoComplete="address-level2" defaultValue={user?.address?.city} required aria-invalid={!!errors.city} aria-describedby={errors.city ? "city-error" : undefined} /></Field>
    </Card>
  );
}
...
export function PaymentChoices({ de, fulfillmentType, paymentMethod, errors, onChange }: { de: boolean; fulfillmentType: FulfillmentType; paymentMethod: PaymentMethod; errors: FieldErrors; onChange: (value: PaymentMethod) => void }) {
  ...
  return (
    <Card className="space-y-4 rounded-none p-5"><fieldset><legend className="mb-3 font-display text-xl font-bold">{de ? "Zahlung" : "Payment"}</legend>
      ...
      </div>{errors.paymentMethod ? <p className="mt-2 text-sm text-destructive">{errors.paymentMethod}</p> : null}
    </fieldset>
      <Field id="note" label={de ? "Bestellnotiz (optional)" : "Order note (optional)"}><textarea id="note" name="note" maxLength={1000} className="min-h-24 w-full rounded-xl border border-border bg-surface p-3 focus:outline-none focus:ring-2 focus:ring-secondary" /></Field>
    </Card>
  );
}
...
export function CheckoutSummary({ de, locale, items, quote, error, errors, busy, refreshQuote }: { de: boolean; locale: "de" | "en"; items: Array<{ key: string; quantity: number; productName: string; unitPriceRappen: number }>; quote: Quote | null; error: string; errors: FieldErrors; busy: boolean; refreshQuote: () => void }) {
  return (
    <Card className="h-fit space-y-4 rounded-none p-5 lg:sticky lg:top-24"><h2 className="font-display text-xl font-bold">{de ? "Zusammenfassung" : "Summary"}</h2>
      ...
      {quote ? <dl className="space-y-2 border-y border-border py-4 text-sm"><div className="flex justify-between"><dt>{de ? "Zwischensumme" : "Subtotal"}</dt><dd>{formatMoney(quote.subtotalRappen, locale)}</dd></div>
        {quote.discountRappen > 0 ? <div className="flex justify-between"><dt>{de ? "Rabatt" : "Discount"}</dt><dd>−{formatMoney(quote.discountRappen, locale)}</dd></div> : null}
        <div className="flex justify-between"><dt>{de ? "Lieferung" : "Delivery"}</dt><dd>{formatMoney(quote.deliveryFeeRappen, locale)}</dd></div><div className="flex justify-between pt-2 text-base font-bold"><dt>{de ? "Gesamt" : "Total"}</dt><dd>{formatMoney(quote.totalRappen, locale)}</dd></div>
      </dl> : null}
      {error ? <p role="alert" className="rounded-xl bg-destructive/10 p-3 text-sm text-destructive">{error}</p> : null}
      <Button type="submit" className="w-full" disabled={busy || !quote}>{busy ? de ? "Wird verarbeitet…" : "Processing…" : de ? "Bestellung aufgeben" : "Place order"}</Button>
      <p className="text-xs text-muted">{de ? "Preise, Bestand und Liefergebiet werden serverseitig erneut geprüft." : "Prices, stock, and delivery eligibility are revalidated by the server."}</p>
    </Card>
  );
}

function Field({ id, label, error, className, children }: { id: string; label: string; error?: string; className?: string; children: ReactNode }) {
  return <div className={className}><Label htmlFor={id}>{label}</Label><div className="mt-1">{children}</div>{error ? <p id={`${id}-error`} className="mt-1 text-sm text-destructive">{error}</p> : null}</div>;
}
