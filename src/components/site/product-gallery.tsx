...
export function ProductGallery({
  ...
  if (!selected) {
    return (
      <div className="relative aspect-square overflow-hidden rounded-card border border-border bg-surface-warm">
        <StorefrontImage
          src={null}
          alt={productName}
          fill
    ...
    <div className="min-w-0">
      ...
      {images.length > 1 ? (
        <div
          ...
          aria-label={de ? "Produktbilder" : "Product images"}
        >
          {images.map((image, index) => (
            <button
              ...
              aria-label={
                de
                  ? `Bild ${index + 1} von ${images.length} anzeigen`
