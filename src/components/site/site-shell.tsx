...
export function SiteShell({
  ...
  return (
    <div className="flex min-h-dvh flex-col" lang={locale}>
      <a
        href="#main-content"
        className="sr-only focus:fixed focus:left-4 focus:top-4 focus:z-[100] focus:block focus:rounded-control focus:bg-primary focus:px-5 focus:py-3 focus:font-bold focus:text-white"
      >
      ...
      <header className="sticky top-0 z-50 border-b border-border bg-surface/95 backdrop-blur-xl">
        ...
        <div className="mx-auto flex min-h-20 max-w-7xl items-center gap-3 px-4 sm:px-6">
          <Link
            ...
            aria-label={de ? "Zambiel Startseite" : "Zambiel home"}
          >
            <BrandLogo priority />
          </Link>
          <form
            action={`/${locale}/products`}
            className="mx-auto hidden max-w-xl flex-1 sm:flex"
            role="search"
          >
            ...
            <div className="relative w-full">
              ...
              <input
                ...
                className="min-h-11 w-full rounded-control border border-border bg-background py-2 pl-12 pr-4 text-sm outline-none focus:border-primary"
              />
            </div>
          </form>
          <nav
          ...
          >
            {links.map((link) => (
              <Link
                ...
                className={`inline-flex min-h-11 items-center rounded-control px-3 text-sm font-bold ${pathname?.startsWith(link.href) ? "bg-surface-warm text-primary" : "hover:bg-surface-warm"}`}
              >
                {link.label}
              </Link>
            ))}
          ...
          <Link
            ...
            className="hidden min-h-11 items-center gap-2 rounded-control px-3 text-sm font-bold hover:bg-surface-warm md:flex"
          ...
          >
            ...
            {count ? (
              <span className="absolute -right-1 -top-1 inline-flex h-5 min-w-5 items-center justify-center rounded-full bg-secondary px-1 text-[11px] font-bold text-secondary-foreground">
                {count}
              </span>
            ) : null}
          </Link>
          <div ref={accountRef} className="relative">
            <button
              ...
              className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-control border border-border"
            ...
            {accountOpen ? (
              <div
                role="menu"
                className="absolute right-0 top-12 w-64 rounded-card border border-border bg-surface p-2 shadow-2xl"
              >
                {user ? (
                  <>
                    <div className="rounded-control bg-surface-warm p-3">
                    ...
                    <Link
                      ...
                      className="mt-2 flex min-h-11 items-center gap-3 rounded-control px-3 text-sm font-bold hover:bg-surface-warm"
                    >
                      <User
                    ...
                    <Link
                      ...
                      onClick={() => setAccountOpen(false)}
                      className="flex min-h-11 items-center gap-3 rounded-control px-3 text-sm font-bold hover:bg-surface-warm"
                    >
                      <ShoppingBag
                        className="h-4 w-4 text-secondary"
                        aria-hidden="true"
                    ...
                    <button
                      ...
                      className="flex min-h-11 w-full items-center gap-3 rounded-control px-3 text-left text-sm font-bold text-destructive hover:bg-destructive/10"
                    >
                  ...
                  <div className="grid gap-2 p-2">
                    <Link
                      role="menuitem"
                      href={`/${locale}/login`}
                      className="inline-flex min-h-11 items-center justify-center rounded-control bg-primary font-bold text-white"
                    >
                      {labels.login}
                    </Link>
                    <Link
                      ...
                      className="inline-flex min-h-11 items-center justify-center rounded-control border border-primary font-bold text-primary"
                    >
                      {labels.register}
                    </Link>
                  </div>
          ...
          <button
            ...
            className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-control border border-border lg:hidden"
          ...
          >
            ...
            >
              ...
              <input
                ...
                type="search"
                placeholder={labels.search}
                className="min-h-12 w-full rounded-control border border-border bg-background px-4"
              />
            </form>
            <div className="grid gap-1">
              {links.map((link) => (
                <Link
                  ...
                  className="flex min-h-12 items-center rounded-control px-4 font-bold hover:bg-surface-warm"
                >
                  {link.label}
                </Link>
              ))}
              <Link
                href={otherPath}
                hrefLang={otherLocale}
                className="flex min-h-12 items-center gap-3 rounded-control px-4 font-bold hover:bg-surface-warm"
              >
      ...
      <footer className="border-t-4 border-secondary bg-primary text-white">
        <div className="mx-auto grid max-w-7xl gap-10 px-5 py-16 sm:px-8 lg:grid-cols-[1.35fr_repeat(3,1fr)]">
          ...
          <div>
            <h2 className="text-xs font-bold uppercase tracking-[0.16em] text-secondary-light">
              {de ? "Bedingungen & Richtlinien" : "Terms & policies"}
            </h2>
          ...
          <div>
            ...
            <p className="mt-4 text-sm text-white/65">
              {de ? "Fragen? Schreiben Sie an" : "Got questions? Send email at"}
            </p>
            {publicConfig.email ? (
              <a
                href={`mailto:${publicConfig.email}`}
                className="mt-2 inline-block break-all font-bold hover:text-secondary-light"
              >
                {publicConfig.email}
              </a>
            ) : null}
