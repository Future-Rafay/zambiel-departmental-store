...
import { archiveRetailCategory, archiveRetailProduct, archiveRetailVariant } from "@/app/admin/(protected)/retail-actions";
...
export function ArchiveProductDialog({ id }: { id: string }) {
  return <Dialog><DialogTrigger asChild><Button variant="destructive">Archive product</Button></DialogTrigger><DialogContent><DialogTitle>Archive product</DialogTitle><DialogDescription>The product leaves the storefront but remains available to historical orders and audit records.</DialogDescription><form action={archiveRetailProduct} className="mt-6"><input type="hidden" name="id" value={id} /><div className="flex flex-wrap justify-end gap-3"><DialogClose asChild><Button variant="outline">Keep product</Button></DialogClose><Button type="submit" variant="destructive">Confirm archive</Button></div></form></DialogContent></Dialog>;
}

export function ArchiveCategoryDialog({ id }: { id: string }) {
  return <Dialog><DialogTrigger asChild><Button variant="destructive">Archive category</Button></DialogTrigger><DialogContent><DialogTitle>Archive category</DialogTitle><DialogDescription>Archiving is blocked while this category has active products or child categories.</DialogDescription><form action={archiveRetailCategory} className="mt-6"><input type="hidden" name="id" value={id} /><div className="flex flex-wrap justify-end gap-3"><DialogClose asChild><Button variant="outline">Keep category</Button></DialogClose><Button type="submit" variant="destructive">Confirm archive</Button></div></form></DialogContent></Dialog>;
}

export function ArchiveVariantDialog({ id, productId }: { id: string; productId: string }) {
  return <Dialog><DialogTrigger asChild><Button variant="destructive">Archive variant</Button></DialogTrigger><DialogContent><DialogTitle>Archive variant</DialogTitle><DialogDescription>The variant becomes unavailable but remains attached to historical order snapshots and inventory activity.</DialogDescription><form action={archiveRetailVariant} className="mt-6"><input type="hidden" name="id" value={id} /><input type="hidden" name="productId" value={productId} /><div className="flex flex-wrap justify-end gap-3"><DialogClose asChild><Button variant="outline">Keep variant</Button></DialogClose><Button type="submit" variant="destructive">Confirm archive</Button></div></form></DialogContent></Dialog>;
}
