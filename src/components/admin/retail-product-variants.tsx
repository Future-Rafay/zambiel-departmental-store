...
import { ArchiveVariantDialog } from "@/components/admin/danger-actions";
    ...
    <section className="mt-8 space-y-4" aria-labelledby="variants">
      ...
      <Card className="p-5">
        ...
        <form action={generateVariantMatrix} className="grid gap-4 sm:grid-cols-3">
          ...
          <Field label="Price (rappen)" name="priceRappen" type="number" min="0" required />
      ...
      {product.variants.map((variant) => (
        <Card key={variant.id} className="p-5">
          ...
          <form action={saveRetailVariant} className="grid gap-4 sm:grid-cols-3">
            ...
            <Field label="Price (rappen)" name="priceRappen" type="number" min="0" defaultValue={variant.priceRappen} required />
            <Field label="Compare-at (rappen)" name="compareAtPriceRappen" type="number" min="0" defaultValue={variant.compareAtPriceRappen ?? ""} />
