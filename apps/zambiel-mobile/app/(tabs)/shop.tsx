...
} from "../../src/discover-filter-sheet";
...
export default function ShopScreen() {
  ...
  const queryString = useCallback(
    (nextPage: number) => {
      ...
      if (filters.minPriceRappen !== undefined)
        value.set("minPriceRappen", String(filters.minPriceRappen));
      if (filters.maxPriceRappen !== undefined)
        value.set("maxPriceRappen", String(filters.maxPriceRappen));
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      setProducts((current) => {
        ...
        return [
          ...current,
          ...result.items.filter((product) => !existing.has(product.id)),
        ];
      });
      setPage(result.page);
      setPageCount(result.pageCount);
      ...
      if (version === requestVersion.current)
        setLoadMoreError(
          de
            ? "Weitere Produkte konnten nicht geladen werden."
  ...
  const activeFilterCount =
    ...
    Number(filters.minPriceRappen !== undefined) +
    Number(filters.maxPriceRappen !== undefined) +
  ...
  const activeChips = useMemo(() => {
    ...
    if (filters.category)
      chips.push({
        key: "category",
        label: selectedCategory?.name ?? filters.category,
        clear: () =>
    ...
    if (filters.minPriceRappen !== undefined)
      chips.push({
        key: "min",
        label: `CHF ${(filters.minPriceRappen / 100).toFixed(2)}+`,
        clear: () =>
          setFilters((current) => ({ ...current, minPriceRappen: undefined })),
      });
    if (filters.maxPriceRappen !== undefined)
      chips.push({
        key: "max",
        label: `${de ? "Bis" : "Up to"} CHF ${(filters.maxPriceRappen / 100).toFixed(2)}`,
        clear: () =>
          setFilters((current) => ({ ...current, maxPriceRappen: undefined })),
      });
    if (filters.sort !== "featured")
      chips.push({
        key: "sort",
        label: sortLabel(filters.sort, de),
        clear: () =>
          setFilters((current) => ({ ...current, sort: "featured" })),
      });
    return chips;
    ...
    <Screen>
      <Header
        ...
        right={
          <Pressable
            ...
            accessibilityLabel={
              de
                ? `Filter öffnen, ${activeFilterCount} aktiv`
      ...
      <FlatList
        ...
        contentContainerStyle={[
          ...
          !products.length && styles.listEmpty,
        ]}
        ListHeaderComponent={
          <View style={styles.listHeader}>
            <View style={styles.searchWrap}>
              ...
              <TextInput
                accessibilityLabel={de ? "Produkte suchen" : "Search products"}
                value={query}
              ...
              {query ? (
                <Pressable
                  accessibilityRole="button"
                  accessibilityLabel={de ? "Suche löschen" : "Clear search"}
                  onPress={() => setQuery("")}
                  hitSlop={8}
            ...
            {activeChips.length ? (
              <FlatList
                ...
                renderItem={({ item }) => (
                  <Pressable
                    accessibilityRole="button"
                    accessibilityLabel={`${item.label}, ${de ? "entfernen" : "remove"}`}
                    onPress={item.clear}
                    style={({ pressed }) => [
                      styles.activeChip,
                      pressed && styles.pressed,
          ...
          !loading ? (
            <StateView
              ...
              message={
                error ||
                (de
                  ? "Ändere deine Suche oder Filter und versuche es erneut."
                  : "Try changing your search or filters.")
              }
              action={
                error ? (
                  <Button onPress={() => void loadFirstPage()}>
                    {de ? "Erneut versuchen" : "Try again"}
                  </Button>
                ) : activeFilterCount ? (
                  <Button onPress={() => setFilters(defaultFilters)}>
        ...
        ListFooterComponent={
          <ListFooter
            loading={loadingMore}
            error={loadMoreError}
            atEnd={!loading && products.length > 0 && page >= pageCount}
            de={de}
...
function ProductSkeletons({ cardWidth }: { cardWidth: number }) {
  return (
    <View accessibilityLabel="Loading products" style={styles.skeletonGrid}>
    ...
    return (
      <View style={styles.footer}>
        ...
        <Text style={styles.footerText}>
          {de ? "Weitere Produkte werden geladen …" : "Loading more products…"}
        </Text>
      </View>
    );
  if (error)
  ...
  if (atEnd)
    return (
      <Text style={styles.endText}>
        {de ? "Du hast alle Produkte gesehen." : "You’ve seen all products."}
      </Text>
    );
  return null;
}
