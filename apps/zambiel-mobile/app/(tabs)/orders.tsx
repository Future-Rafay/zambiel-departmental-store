...
export default function OrdersScreen() {
  ...
  const query = useQuery({
    ...
    queryFn: () => (user ? api.orders() : guestOrders()),
    staleTime: 30_000,
  });
  useFocusEffect(
    useCallback(() => {
      ...
      <FlatList
        ...
        ListEmptyComponent={
          query.isLoading ? (
            ...
            >
              ...
              <Text style={{ color: colors.muted }}>{copy.loading}</Text>
            </View>
          ) : query.isError ? (
            <View
              style={{
            ...
            >
              ...
              >
                <PackageOpen size={36} color={colors.primary} />
              </View>
              <Text
                accessibilityRole="header"
                style={{
              ...
              >
                {locale === "de" ? "Noch keine Bestellungen" : "No orders yet"}
              </Text>
              <Text
                style={{
                  color: colors.muted,
              ...
              >
                {locale === "de"
                  ? "Entdecke praktische Produkte und verfolge deine Bestellung später hier."
                  : "Find something useful and track every order here."}
              </Text>
              <Button onPress={() => router.push("/(tabs)/shop")}>
                ...
                >
                  ...
                  >
                    {locale === "de" ? "Produkte entdecken" : "Start shopping"}
                  </Text>
                </View>
              </Button>
            </View>
        ...
        renderItem={({ item }) => (
          <Pressable
            ...
            accessibilityLabel={`${item.orderNumber}, ${statusLabel(item.status, locale)}, ${money(item.totalRappen, locale)}`}
