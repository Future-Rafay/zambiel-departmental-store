...
export function addCartLines(current: CartLine[], additions: CartLine[]) {
  ...
  for (const line of additions) {
    ...
    if (existing)
      existing.quantity = Math.min(20, existing.quantity + line.quantity);
    else if (next.length < 100)
      next.push({ ...line, quantity: Math.min(20, line.quantity) });
  }
  return next;
}
  ...
  return addCartLines(
    ...
    value.filter(
      (line): line is CartLine =>
        ...
        Number.isSafeInteger(line.priceRappen) &&
        line.priceRappen >= 0 &&
        Number.isInteger(line.quantity) &&
        line.quantity > 0 &&
  ...
  return product.variants.find((variant) =>
    product.options.every((option) =>
      variant.optionValues.some(
        (value) =>
          value.optionId === option.id && value.valueId === choices[option.id],
      ),
    ),
  );
}
...
export function addRecentlyViewed(
  ...
  const snapshot = {
    ...
    minimumPriceRappen: product.minimumPriceRappen,
  };
  return [snapshot, ...current.filter((item) => item.id !== product.id)].slice(
    0,
    8,
  );
}
export function readRecentlyViewed(value: unknown): ProductPreview[] {
  return Array.isArray(value)
    ? value
        .filter(
          (item): item is ProductPreview =>
            !!item &&
            typeof item.id === "string" &&
            typeof item.slug === "string" &&
            typeof item.name === "string" &&
            (item.imageUrl === null || typeof item.imageUrl === "string") &&
            Number.isSafeInteger(item.minimumPriceRappen) &&
            item.minimumPriceRappen >= 0,
        )
        .slice(0, 8)
    : [];
}
export function paymentForFulfillment(
  current: CheckoutInput["paymentMethod"],
  fulfillment: CheckoutInput["fulfillmentType"],
...
const statuses: Record<string, [string, string]> = {
  PAYMENT_PENDING: ["Zahlung ausstehend", "Awaiting payment"],
