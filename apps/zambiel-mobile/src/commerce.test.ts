...
test("cart merges variants and respects authoritative quantity and line limits", () => {
  const line: CartLine = { variantId: "v1", productId: "p1", slug: "product", name: "Product", variant: "Default", priceRappen: 1295, imageUrl: null, quantity: 15 };
  assert.equal(addCartLines([line], [{ ...line, quantity: 12 }])[0].quantity, 20);
  assert.equal(line.quantity, 15);
  assert.equal(readCart([{ ...line, quantity: -1 }, { ...line, priceRappen: NaN }, line]).length, 1);
  assert.equal(addCartLines([], Array.from({ length: 105 }, (_, i) => ({ ...line, variantId: `v${i}` }))).length, 100);
});
test("price filters retain blank versus zero and reject invalid money", () => {
  assert.equal(priceFilter(""), undefined); assert.equal(priceFilter("0"), 0); assert.equal(priceFilter("12,95"), 1295);
  assert.throws(() => priceFilter("1e4")); assert.throws(() => priceFilter("1.234"));
});
