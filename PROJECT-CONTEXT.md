...
- Fulfillment: delivery or pickup. Delivery supports Stripe/COD; pickup supports Stripe/cash at pickup. Delivery eligibility, minimum order, CHF fee, and free-shipping threshold are configured once per destination country; checkout does not collect a postal code.
- Excluded: vendors, tenants, organizations, commissions, payouts, order splitting, warehouses, backorders, reviews, and partial-refund restocking.
...
- Customer Android development is authorized in `apps/zambiel-mobile` with React Native/Expo; see `MOBILE-APP.md`. The previous staff application was already deleted before this work. Retained staff API endpoints are not customer-app functionality.
...
- Server code reprices lines and validates stock, promotions, destination-country shipping rules, fulfillment, and payment methods.
- Orders, fees, promotions, payments, and refunds remain authoritative in CHF integer rappen. Stripe Checkout enables Adaptive Pricing so supported customers may see and pay the CHF order in local currency; Zambiel does not store exchange rates or mixed-currency order totals.
...
- Country-shipping verification (2026-09-09): 44 tests (39 passed, 5 isolated-database skips), Prisma validation/generation, typecheck, production build, migration deployment/status, and diff whitespace checks passed. Lint has no errors and one unrelated unused-import warning in the user-edited category card. Live checkout checks confirmed no postal field, CHF 7.90 shipping below CHF 120, free shipping at CHF 120, and no 375px horizontal overflow. Authenticated admin and live Stripe local-currency presentment remain environment-dependent checks.

- Ten migrations define the independent Zambiel schema. The forward-only country-shipping and customer-mobile migrations are applied to local `zambiel_dev`; migration status is current.
...
- Demo configuration now includes Switzerland at CHF 7.90 with free shipping from CHF 120. The guarded seed applied this configuration and nullable-postal address updates, then stopped on an existing zero-stock demo movement; production stock safeguards were not weakened.
- The header announcement bar rotates through every active shipping country's localized delivery minimum and free-shipping threshold. Automatic rotation stops when reduced motion is requested.
- Local Stripe Checkout requires `npm run stripe:listen` beside the backend and the listener's signing secret in `STRIPE_WEBHOOK_SECRET`; without the listener, Stripe can collect a test payment but the signature-verified order finalizer cannot receive it. Stripe Dashboard Test mode must be enabled to view test-key payments.
- Stripe diagnosis (2026-09-10): three test-mode Checkout Sessions were recovered through the signature-verified webhook route and are now `CONFIRMED` with `PAID` payments. The latest recovery returned HTTP 200, stored its PaymentIntent, consumed one tracked inventory reservation exactly once, and an identical signed replay produced no duplicate movement, status event, webhook claim, or notification. Stripe CLI 1.50.10 is installed locally and the listener/backend were running after verification; future sessions still require the listener or a public HTTPS endpoint.
...
- Superseded product decision (2026-08-31): no React Native application. On 2026-09-09 the user authorized a new customer-only Android app; old staff-app verification remains unrelated.
...
- The former staff app was deleted before customer-app work. Retained staff-mobile API compatibility code stays separate; do not reuse its authorization for customer mobile access.
