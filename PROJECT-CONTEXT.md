...
- Local Stripe Checkout requires `stripe listen --forward-to localhost:3000/api/webhooks/stripe` and the listener's signing secret in `STRIPE_WEBHOOK_SECRET`; without the listener, Stripe can collect a test payment but the signature-verified order finalizer cannot receive it. Stripe Dashboard Test mode must be enabled to view test-key payments.
- Stripe diagnosis (2026-09-09): two recent test-mode Checkout Sessions were complete and paid, but no Stripe webhook endpoint/listener existed. The pending customer-mobile migration initially made controlled webhook replay fail on its missing push-subscription table. After applying the migration, both signed replays returned HTTP 200 and both orders became `CONFIRMED` with `PAID` payments. This verifies local recovery, not live webhook delivery.
